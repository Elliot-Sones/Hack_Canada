...
async def create_entitlement_run(
    ...
    result = EntitlementResult(scenario_run_id=scenario_id, overall_compliance="pending", result_json={})
    db.add(result)
    await db.flush()
...
async def get_precedent_search(
    ...
    result = await db.execute(select(PrecedentSearch).where(PrecedentSearch.id == search_id))
