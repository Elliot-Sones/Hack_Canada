...
- **Policy hierarchy**: Planning Act → PPS 2024 → Greenbelt Plan → Toronto OP → Zoning By-law 569-2013 → Site Plan Control
- **Four variance tests** (all four must pass): minor in nature / desirable for land / general intent of ZBL maintained / general intent of OP maintained
- **CoA cannot vary OBC**: fire access path (0.9m min, 45m max travel), limiting distances, fire-resistance ratings — these require an Alternative Solution to the Chief Building Official
- **O.Reg 462/24**: removes FSI restrictions and sets lot coverage at 45% max for multiplexes (≤3 units after addition); does NOT override setbacks or frontage
- **Bill 185 (2024)**: zero minimum parking for multiplexes; SPC exempt for ≤10 unit residential
- **Bill 60 (2025)**: as-of-right variances for prescribed residential deviations (no CoA needed); lapsing provisions on site plan approvals

---

## Data Ingestion

Real data from Toronto Open Data is ingested via CKAN API:

```bash
# Trigger via API (requires auth token)
POST /api/v1/admin/ingest/building-permits
...
- Compliance engine (`app/services/compliance_engine.py`) is **deterministic — no AI**. Never add AI calls to it.
- All AI-generated documents get `SAFETY_PREAMBLE` prepended automatically.
- The `_GROUNDING_INSTRUCTION` in templates.py tells the AI to cite only references provided in context — enforced via `ontario_policy.py` constants.
- Zoning standards are in `app/data/toronto_zoning.py` (hardcoded). Policy narrative is in `app/data/ontario_policy.py`.
- `DevelopmentApplication.decision` is the critical field for precedent scoring — NULL means no real data.
