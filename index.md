...
| `src/components/floorplan/FloorPlanEditor.jsx` | 2D Konva-based floor plan editor container | Pan/zoom Stage, tool toolbar (select/wall/room/door/window/delete/measure), undo/redo (Ctrl+Z), debounced compliance API calls, wall center drag (move entire walls), delete tool (press X or click delete button), imports real layer components (WallLayer/RoomLayer/OpeningLayer/DimensionLayer) plus ComplianceBadgeLayer. |
...
| `src/components/floorplan/panels/EditorToolbar.jsx` | Editor tool palette | Vertical left toolbar: Select (V), Wall (W), Door (D), Window (O), Room Type (R), Delete (X), Measure (M) tools; undo/redo buttons (Ctrl+Z/Ctrl+Y); dimension toggle; uses model-ctrl-btn CSS class. |
...
*Last updated: 2026-03-08 (Improve floor plan UX: add wall center drag handle for moving entire walls, delete tool (eraser mode) with keyboard shortcut X, hover visual feedback on walls, improved wall selection interaction)*
